 ...
 export default function Home() {
  ...
  return (
    ...
    <div className='flex  justify-center w-full mt-4 sm:h-[55px] h-[35px]'>
       ...
       <div className='absolute shadow-lg bg-white z-[900]  top-[60px] left-0 min-h-[70px] w-full rounded-md'>
       <div className='pt-[20px] px-[50px]'>
       {resultArr.length>0?resultArr.map((obj)=>{
        return (
          <>
          <p className='py-3 cursor-pointer' onClick={()=>setWantedData(obj)} key={obj.id}>{obj.name}</p>
          <hr key={obj.id}/>
          </>
        )
       ...
       <input className='rounded-[3px] w-full h-full sm:rounded-[10px] border-2 px-4  border-gray-300  text-[12px] sm:text-base' onChange={((e)=>setSearchValue(e.target.value))} placeholder='Search by name' /></div>
